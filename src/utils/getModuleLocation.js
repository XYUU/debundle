const { getAllPathsToModule } = require('./getModulePath');
...
function getModuleLocation(
  ...
  if (rootNodeModule) {
    ...
    if (modulePath.endsWith(path.sep)) {
  ...
  let notMatch = !filePath.startsWith(pathPrefix);
  if (notMatch) {
    console.log(`"${mod.id}":"${modulePath}",${pathMapping[mod.id]}`);
  }
  if (notMatch || modulePath.indexOf("undefined") !== -1) {
    // console.log(`${mod.id} => ${modulePath} --- ${str}`);
    let str = pathMapping[mod.id];
    if (str === undefined && mod.id !== entryPointModuleId) {
      console.log(`****"${mod.id}":"${modulePath}",${str}`);
    }
  }
  return filePath;
